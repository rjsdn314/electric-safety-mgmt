...
export function InspectionForm() {
  ...
  const selectFolder = async () => {
    try {
      ...
      alert(`폴더 저장 완료: ${handle.name}\n\n이제부터 모든 점검은 이 폴더에 자동 저장됩니다.\n다음에 접속해도 같은 폴더가 자동으로 사용됩니다!`);
    } catch (e: any) {
        ...
        <div style={{display:'flex',alignItems:'center',gap:10,minWidth:0,flex:1}}>
          ...
          <div style={{minWidth:0,flex:1}}>
            ...
            <div style={{fontSize:11,color:'var(--text-secondary)',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>
              {folderName ? `✓ ${folderName} (자동저장)` : '폴더 선택 후 자동 저장됩니다'}
            </div>
          </div>
        </div>
      ...
      <div className="toss-card" style={{display:'flex',flexDirection:'column',gap:24}}>
        ...
        {selected && (
          <div style={{padding:'10px 14px',background:'var(--bg-elevated)',borderRadius:10,fontSize:13}}>
            ...
            <span style={{color:'var(--accent)',fontWeight:600}}>{selected.name}_{inspType}점검_{date.replace(/-/g,'')}.xlsx</span>
          </div>
        )}
