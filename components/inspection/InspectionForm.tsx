        ...
        <>
              <div className="no-print" style={{ maxWidth: 940, margin: 0, padding: 0 }}>
                      <div style={{ background: 'linear-gradient(135deg,rgba(5,192,114,.08),rgba(49,130,246,.08))', border: '1px solid rgba(5,192,114,.3)', borderRadius: 20, padding: 40, textAlign: 'center' }}>
                        ...
                        {folderName && (<div style={{ fontSize: 13, color: 'var(--text-secondary)', marginBottom: 24 }}>💾 로컬 저장: {folderName}</div>div>)}
                                <div style={{ display: 'grid', gap: 10, maxWidth: 400, margin: '0 auto 10px' }}>
                                  {downloadUrl && (
                        <a href={downloadUrl} download={savedFile} style={{ display: 'block', background: 'var(--accent)', color: '#fff', border: 'none', borderRadius: 12, padding: '16px 24px', fontSize: 15, fontWeight: 700, textDecoration: 'none' }}>⬇️ 엑셀 저장 (.xlsx)</a>a>
                                            )}
                                            <button onClick={handlePrintPdf} style={{ display: 'block', width: '100%', background: 'var(--bg-elevated)', color: 'var(--text-primary)', border: '1.5px solid var(--accent)', borderRadius: 12, padding: '16px 24px', fontSize: 15, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>🖨️ PDF 저장 (인쇄 → PDF로 저장)</button>button>
                                  {downloadUrl && (
                        <button onClick={() => { const a = document.createElement('a'); a.href = downloadUrl; a.download = savedFile; document.body.appendChild(a); a.click(); a.remove(); setTimeout(handlePrintPdf, 600); }} style={{ display: 'block', width: '100%', background: 'rgba(49,130,246,.12)', color: 'var(--accent)', border: 'none', borderRadius: 12, padding: '16px 24px', fontSize: 15, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>📦 엑셀 + PDF 저장</button>button>
                                ...
                                <button onClick={() => { setDone(false); setSelected(null); setQuery(''); setMeasureSets([emptyMeasureSet()]); setRemarks(''); setResult(null); }} style={{ width: '100%', maxWidth: 400, background: 'transparent', color: 'var(--text-secondary)', border: '1px solid var(--border)', borderRadius: 12, padding: '14px 24px', fontSize: 14, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit', marginTop: 8 }}>새 점검 생성</button>button>
                      </div>div>
              </div>div>
          ...
          <div style={{ maxWidth: 940, margin: 0, padding: 0 }}>
                <div style={{ background: 'var(--bg-card)', borderRadius: 16, padding: 18, display: 'flex', alignItems: 'center', gap: 14, marginBottom: 14 }}>
                        ...
                        <div style={{ minWidth: 0, flex: 1 }}>
                                  ...
                                  <div style={{ fontSize: 12, color: 'var(--text-secondary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{folderName ? (selected ? `${folderName} / ${selected.name}` : folderName) : '⚠️ 저장 폴더가 지정되지 않았습니다 (다운로드만 가능)'}</div>div>
                        </div>div>
                        <button onClick={selectFolder} style={{ padding: '10px 18px', borderRadius: 10, border: 'none', background: 'var(--accent)', color: '#fff', fontSize: 13, fontWeight: 700, cursor: 'pointer', flexShrink: 0, fontFamily: 'inherit' }}>{folderHandle ? '변경' : '선택'}</button>button>
                </div>div>
                                                              ...
                                                              <button key={s.id} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 10, padding: '12px 14px', borderRadius: 10, border: 'none', background: today ? 'rgba(5,192,114,.08)' : 'transparent', color: 'var(--text-primary)', cursor: 'pointer', textAlign: 'left' }} onClick={() => handleSelectStation(s)}>
                                                                                  ...
                                                                                  <div style={{ flex: 1, minWidth: 0 }}>
                                                                                                        <div style={{ fontSize: 14, fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                                                                                                          {today && <span style={{ marginRight: 6, fontSize: 10, fontWeight: 800, padding: '1px 6px', borderRadius: 6, background: 'rgba(5,192,114,.18)', color: '#05a862' }}>📅 오늘</span>span>}
                                                                                                          {s.name}
                                                                                                          </div>div>
                  ...
                  {selected && (
                      <div style={{ marginTop: 14, padding: 18, background: 'var(--bg-elevated)', borderRadius: 12 }}>
                                  ...
                                  <div style={{ display: 'flex', gap: 24, fontSize: 13, color: 'var(--text-secondary)', flexWrap: 'wrap' }}>
                                    ...
                                    {selected.panel_count > 1 && (<div><span style={{ opacity: 0.7 }}>수배전반</span>span> <strong style={{ color: 'var(--accent)', marginLeft: 6 }}>{selected.panel_count}개</strong>strong></div>div>)}
                                  </div>div>
                      </div>div>
                        )}
                </div>div>
                ...
                <div style={{ background: 'var(--bg-card)', borderRadius: 16, padding: 24, marginBottom: 14 }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 14 }} className="info-grid">
                                  <div><label style={labelStyle}>점검일자</label>label><input type="date" className="toss-input" value={date} onChange={e => setDate(e.target.value)} /></div>div>
                                  <div><label style={labelStyle}>점검자</label>label><input className="toss-input" placeholder="이름" value={inspector} onChange={e => setInspector(e.target.value)} /></div>div>
                                  <div><label style={labelStyle}>점검횟수</label>label><input type="number" className="toss-input" min={1} value={count} onChange={e => setCount(Number(e.target.value))} /></div>div>
                                  <div><label style={labelStyle}>날씨(일기)</label>label>
                                              <select className="toss-input" value={weather} onChange={e => setWeather(e.target.value)}>
                                                            <option value="자동">자동(실제 날씨)</option>option>
                                                            <option value="맑음">맑음</option>option>
                                                            <option value="흐림">흐림</option>option>
                                                            <option value="우천">우천</option>option>
                                              </select>select>
                                  </div>div>
                        </div>div>
                </div>div>
                ...
                <div style={{ background: 'var(--bg-card)', borderRadius: 16, padding: 24, marginBottom: 14 }}>
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
                                  ...
                                  <button onClick={addMeasureSet} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 16px', borderRadius: 10, border: '1.5px dashed var(--accent)', background: 'var(--accent-soft)', color: 'var(--accent)', fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>+ 개소 추가</button>button>
                        </div>div>
                  {measureSets.map((set, idx) => (
                      <div key={idx} style={{ marginBottom: idx < measureSets.length - 1 ? 28 : 0 }}>
                                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
                                                <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--accent)', padding: '4px 12px', borderRadius: 8, background: 'var(--accent-soft)' }}>수배전반 #{idx + 1}</div>div>
                                    {measureSets.length > 1 && (<button onClick={() => removeMeasureSet(idx)} style={{ padding: '4px 12px', borderRadius: 8, border: '1px solid var(--border)', background: 'transparent', color: 'var(--text-secondary)', fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>✕ 제거</button>button>)}
                                  </div>div>
                        ...
                        {(inspType === '반기' || inspType === '연차') && (
                                      <div style={{ marginBottom: 14 }}>
                                                      <label style={labelStyle}>접지저항 (Ω) — 별지2-접지저항 D{5 + idx}</label>label>
                                                      <div style={{ position: 'relative', maxWidth: 220 }}>
                                                                        ...
                                                                        <span style={{ position: 'absolute', right: 14, top: '50%', transform: 'translateY(-50%)', fontSize: 13, color: 'var(--text-secondary)', pointerEvents: 'none', fontWeight: 600 }}>Ω</span>span>
                                                      </div>div>
                                      </div>div>
                                  )}
                                  <div>
                ...
                <div style={{ background: 'var(--bg-card)', borderRadius: 16, padding: 24, marginBottom: 14 }}>
                        <label style={sectionTitle}>종합 의견(별지14)</label>label>
            ...
            {selected && (
                    <div style={{ padding: 16, background: 'var(--bg-elevated)', borderRadius: 12, fontSize: 13, marginBottom: 14 }}>
                              ...
                              <div style={{ color: 'var(--accent)', fontWeight: 700, wordBreak: 'break-all' }}>{selected.name}_{inspType}점검_{date.replace(/-/g, '')}.xlsx</div>div>
                    </div>div>
                )}
          
                <button onClick={handleSubmit} disabled={loading} style={{ width: '100%', padding: 18, borderRadius: 14, border: 'none', background: 'var(--accent)', color: '#fff', fontSize: 16, fontWeight: 700, cursor: 'pointer', opacity: loading ? .6 : 1, fontFamily: 'inherit' }}>{loading ? '⏳ 생성 중...' : '⚡ 직무고시 엑셀 생성 및 저장'}</button>button>
...
function PrintableSheet({ data }: { data: any }) {
    ...
    return (
          <div id="print-sheet" style={{ color: '#000', background: '#fff', fontFamily: 'sans-serif' }}>
                <h2 style={{ textAlign: 'center', fontSize: 20, fontWeight: 800, margin: '0 0 4px' }}>전기설비 점검표</h2>h2>
                <div style={{ textAlign: 'center', fontSize: 13, marginBottom: 14 }}>{data.inspType}점검</div>div>
                <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: 14 }}>
                        <tbody>
                                  <tr><td style={labelTd}>충전소명</td>td><td style={td} colSpan={3}>{st.name || '-'}</td>td></tr>tr>
                                  <tr>
                                              <td style={labelTd}>수전전압</td>td><td style={td}>{st.voltage}V ({isHighV ? '고압' : '저압'})</td>td>
                                              <td style={labelTd}>수전용량</td>td><td style={td}>{st.capacity}kW</td>td>
                                  </tr>tr>
                                  <tr>
                                              <td style={labelTd}>점검일자</td>td><td style={td}>{data.date}</td>td>
                                              <td style={labelTd}>점검자</td>td><td style={td}>{data.inspector}</td>td>
                                  </tr>tr>
                                  <tr>
                                              <td style={labelTd}>점검횟수</td>td><td style={td}>{data.count}</td>td>
                                              <td style={labelTd}>날씨</td>td><td style={td}>{data.weather}</td>td>
                                  </tr>tr>
                        </tbody>tbody>
                </table>table>
                <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: 14 }}>
                        <thead>
                                  <tr>
                                              <th style={th} rowSpan={2}>수배전반</th>th>
                                              <th style={th} colSpan={4}>전압 (V)</th>th>
                                              <th style={th} colSpan={3}>전류 (A)</th>th>
                                    {(data.inspType === '반기' || data.inspType === '연차') && <th style={th} rowSpan={2}>접지저항 (Ω)</th>th>}
                                  </tr>tr>
                                  <tr>
                                              <th style={th}>A</th>th><th style={th}>B</th>th><th style={th}>C</th>th><th style={th}>N</th>th>
                                              <th style={th}>A</th>th><th style={th}>B</th>th><th style={th}>C</th>th>
                                  </tr>tr>
                        </thead>thead>
                        <tbody>
                          {sets.map((s, i) => (
                        <tr key={i}>
                                      <td style={labelTd}>#{i + 1}</td>td>
                                      <td style={td}>{s.voltage_A}</td>td><td style={td}>{s.voltage_B}</td>td><td style={td}>{s.voltage_C}</td>td><td style={td}>{s.voltage_N}</td>td>
                                      <td style={td}>{s.current_A}</td>td><td style={td}>{s.current_B}</td>td><td style={td}>{s.current_C}</td>td>
                          {(data.inspType === '반기' || data.inspType === '연차') && <td style={td}>{s.ground}</td>td>}
                        </tr>tr>
                      ))}
                        </tbody>tbody>
                </table>table>
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                        <tbody>
                          {sets.map((s, i) => (
                        <tr key={i}><td style={labelTd}>특이사항 #{i + 1}</td>td><td style={{ ...td, textAlign: 'left' }}>{s.remarks || '특이사항 없음'}</td>td></tr>tr>
                      ))}
                                  <tr><td style={labelTd}>종합의견</td>td><td style={{ ...td, textAlign: 'left', height: 50, verticalAlign: 'top' }}>{data.remarks || '특이사항 없음'}</td>td></tr>tr>
                        </tbody>tbody>
                </table>table>
                <div style={{ textAlign: 'right', marginTop: 24, fontSize: 13 }}>점검자: {data.inspector} (인)</div>div>
          </div>div></div>
        );
}
