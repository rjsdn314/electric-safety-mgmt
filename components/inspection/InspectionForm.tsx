        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:10,marginBottom:18}}>
          {TYPE_OPTIONS.map(t=>(
            <button key={t.value} onClick={()=>setInspType(t.value)}
              style={{
                padding:'16px 4px',borderRadius:12,
                ...
                fontSize:15,fontWeight:700,cursor:'pointer',textAlign:'center',fontFamily:'inherit'
              }}>
              {t.value}
            ...
            <div className="measure-grid" style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:10}}>
              {['A상','B상','C상','N'].map((ph,i)=>(
                <div key={ph} style={{position:'relative'}}>
                  ...
                  <span style={{position:'absolute',right:14,top:'50%',transform:'translateY(-50%)',fontSize:13,color:'var(--text-secondary)',pointerEvents:'none',fontWeight:600}}>{row.unit}</span>
                </div>
              ))}
            </div>
          </div>
